...
import org.ziyao.data.elasticsearch.core.MultiGetItem;
...
class ResponseConverter {
	...
	public Settings indicesGetSettings(GetIndicesSettingsResponse getIndicesSettingsResponse, String indexName) {
		...
		if (indexState != null) {
			...
			Function<IndexSettings, Settings> indexSettingsToSettings = indexSettings -> {
				Settings parsedSettings = Settings.parse(JsonUtils.toJson(indexSettings, jsonpMapper));
	...
	public Document indicesGetMapping(GetMappingResponse getMappingResponse, IndexCoordinates indexCoordinates) {
		...
		return Document.parse(JsonUtils.toJson(indexMappingRecord.mappings(), jsonpMapper));
	}
	...
	public List<IndexInformation> indicesGetIndexInformations(GetIndexResponse getIndexResponse) {
		...
		getIndexResponse.result().forEach((indexName, indexState) -> {
			Settings settings = indexState.settings() != null ? Settings.parse(JsonUtils.toJson(indexState.settings(), jsonpMapper))
					: new Settings();
			Document mappings = indexState.mappings() != null ? Document.parse(JsonUtils.toJson(indexState.mappings(), jsonpMapper))
					: Document.create();
	...
	private AliasData indicesGetAliasData(String aliasName, Alias alias) {
		...
		String filterJson = filter != null ? JsonUtils.toJson(filter, jsonpMapper) : null;
	...
	public TemplateData indicesGetTemplateData(GetTemplateResponse getTemplateResponse, String templateName) {
		...
		if (templateMapping != null) {
			...
			Document mapping = Document.parse(JsonUtils.toJson(templateMapping.mappings(), jsonpMapper));
