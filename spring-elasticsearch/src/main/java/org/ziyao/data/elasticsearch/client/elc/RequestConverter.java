...
import org.ziyao.data.elasticsearch.core.index.AliasAction;
import org.ziyao.data.elasticsearch.core.index.AliasActionParameters;
import org.ziyao.data.elasticsearch.core.index.AliasActions;
...
import org.ziyao.data.elasticsearch.core.reindex.Remote;
...
class RequestConverter {
	...
	public ExistsTemplateRequest indicesExistsTemplateRequest(
			org.ziyao.data.elasticsearch.core.index.ExistsTemplateRequest existsTemplateRequest) {
	...
	public DeleteTemplateRequest indicesDeleteTemplateRequest(
			org.ziyao.data.elasticsearch.core.index.DeleteTemplateRequest existsTemplateRequest) {
	...
	public GetTemplateRequest indicesGetTemplateRequest(
			org.ziyao.data.elasticsearch.core.index.GetTemplateRequest getTemplateRequest) {
	...
	public co.elastic.clients.elasticsearch.core.ReindexRequest reindex(org.ziyao.data.elasticsearch.core.reindex.ReindexRequest reindexRequest,
		...
		builder //
				.source(s -> {
					...
					org.ziyao.data.elasticsearch.core.reindex.ReindexRequest.Slice slice = source.getSlice();
					if (slice != null) {
				...
				.dest(d -> {
					org.ziyao.data.elasticsearch.core.reindex.ReindexRequest.Dest dest = reindexRequest.getDest();
					return d //
		...
		org.ziyao.data.elasticsearch.core.reindex.ReindexRequest.Script script = reindexRequest.getScript();
		if (script != null) {
			builder.script(s -> s.inline(InlineScript.of(i -> i.lang(script.getLang()).source(script.getSource()))));
		}

		builder.timeout(TypeUtils.time(reindexRequest.getTimeout())) //
				.scroll(TypeUtils.time(reindexRequest.getScroll()));
	...
	private VersionType retrieveVersionTypeFromPersistentEntity(@Nullable Class<?> clazz) {
		...
		if (persistentEntity != null) {
			org.ziyao.data.elasticsearch.annotations.Document.VersionType entityVersionType = persistentEntity
					.getVersionType();
