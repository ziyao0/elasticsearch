...
import com.ziyao.springframework.data.elasticsearch.core.query.RescorerQuery.ScoreMode;
import com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest;
...
class RequestFactory {
    ...
    public org.elasticsearch.index.reindex.ReindexRequest reindexRequest(ReindexRequest reindexRequest) {
        ...
        // region source build
        final ReindexRequest.Source source = reindexRequest.getSource();
        ...
        final ReindexRequest.Slice slice = source.getSlice();
        if (slice != null) {
        ...
        // region dest build
        final ReindexRequest.Dest dest = reindexRequest.getDest();
        request.setDestIndex(dest.getIndex().getIndexName()).setDestRouting(dest.getRouting())
                .setDestPipeline(dest.getPipeline());

        final com.ziyao.springframework.data.elasticsearch.annotations.Document.VersionType versionType = dest.getVersionType();
        if (versionType != null) {
    ...
    private List<MultiGetRequest.Item> getMultiRequestItems(Query searchQuery, Class<?> clazz, IndexCoordinates index) {
        ...
        if (!CollectionUtils.isEmpty(searchQuery.getIdsWithRouting())) {
            String indexName = index.getIndexName();
    ...
    private QueryBuilder getQuery(Query query) {
        ...
        if (query instanceof NativeSearchQuery) {
            ...
            elasticsearchQuery = QueryBuilders.wrapperQuery(stringQuery.getSource());
        } else {
