...
import com.ziyao.springframework.data.elasticsearch.core.TotalHitsRelation;
import com.ziyao.springframework.data.elasticsearch.core.document.SearchDocument;
import com.ziyao.springframework.data.elasticsearch.core.suggest.response.Suggest;
	...
	public static <T> SearchDocumentResponse from(HitsMetadata<?> hitsMetadata, @Nullable String scrollId,
		...
		if (responseTotalHits != null) {
			...
			switch (responseTotalHits.relation().jsonValue()) {
				case "eq":
					totalHitsRelation = com.ziyao.springframework.data.elasticsearch.core.TotalHitsRelation.EQUAL_TO.name();
					break;
				case "gte":
					totalHitsRelation = com.ziyao.springframework.data.elasticsearch.core.TotalHitsRelation.GREATER_THAN_OR_EQUAL_TO.name();
					break;
				default:
