 ...
 */
package com.ziyao.springframework.data.elasticsearch.client.elc;
...
import com.ziyao.springframework.data.elasticsearch.support.DefaultStringObjectMap;
import jakarta.json.stream.JsonGenerator;
import jakarta.json.stream.JsonParser;
import org.springframework.dao.InvalidDataAccessApiUsageException;
import com.ziyao.springframework.data.domain.Sort;
import com.ziyao.springframework.data.elasticsearch.core.RefreshPolicy;
import com.ziyao.springframework.data.elasticsearch.core.ScriptType;
import com.ziyao.springframework.data.elasticsearch.core.convert.ElasticsearchConverter;
import com.ziyao.springframework.data.elasticsearch.core.index.AliasAction;
import com.ziyao.springframework.data.elasticsearch.core.index.AliasActionParameters;
import com.ziyao.springframework.data.elasticsearch.core.index.AliasActions;
...
import com.ziyao.springframework.data.elasticsearch.core.reindex.Remote;
...
import static com.ziyao.springframework.data.elasticsearch.client.elc.TypeUtils.*;
...
class RequestConverter {
	...
	public co.elastic.clients.elasticsearch.indices.PutTemplateRequest indicesPutTemplateRequest(
			com.ziyao.springframework.data.elasticsearch.core.index.PutTemplateRequest putTemplateRequest) {
	...
	public ExistsTemplateRequest indicesExistsTemplateRequest(
			com.ziyao.springframework.data.elasticsearch.core.index.ExistsTemplateRequest existsTemplateRequest) {

		Assert.notNull(existsTemplateRequest, "existsTemplateRequest must not be null");

		return ExistsTemplateRequest
				.of(etr -> etr.name(existsTemplateRequest.getTemplateName()));
	}

	public DeleteTemplateRequest indicesDeleteTemplateRequest(
			com.ziyao.springframework.data.elasticsearch.core.index.DeleteTemplateRequest existsTemplateRequest) {
	...
	public GetTemplateRequest indicesGetTemplateRequest(
			com.ziyao.springframework.data.elasticsearch.core.index.GetTemplateRequest getTemplateRequest) {
	...
	public co.elastic.clients.elasticsearch.core.ReindexRequest reindex(com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest reindexRequest,
		...
		builder //
				.source(s -> {
					com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest.Source source = reindexRequest.getSource();
					s.index(Arrays.asList(source.getIndexes().getIndexNames())) //
							.size(source.getSize());

					com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest.Slice slice = source.getSlice();
					if (slice != null) {
				...
				.dest(d -> {
					com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest.Dest dest = reindexRequest.getDest();
					return d //
		...
		com.ziyao.springframework.data.elasticsearch.core.reindex.ReindexRequest.Script script = reindexRequest.getScript();
		if (script != null) {
	...
	private VersionType retrieveVersionTypeFromPersistentEntity(@Nullable Class<?> clazz) {
		...
		if (persistentEntity != null) {
			com.ziyao.springframework.data.elasticsearch.annotations.Document.VersionType entityVersionType = persistentEntity
					.getVersionType();
