...
class EvaluationContextExtensionInformation {
    ...
    public EvaluationContextExtensionInformation(Class<? extends EvaluationContextExtension> type) {
        ...
        Class<?> rootObjectType = org.ziyao.data.util.ReflectionUtils.findRequiredMethod(type, "getRootObject")
                .getReturnType();
     ...
     * @return {@literal true} if {@link ExpressionDependencies.ExpressionDependency} is provided by this root object or the extension itself.
     * @since 2.4
     */
    ...
    static class RootObjectInformation {
        ...
        RootObjectInformation(Class<?> type) {
            ...
            ReflectionUtils.doWithFields(type, RootObjectInformation.this.fields::add, ExtensionTypeInformation.PublicMethodAndFieldFilter.NON_STATIC);
        }
        ...
        private MultiValueMap<String, Function> getFunctions(Object target) {
            return methods.stream().collect(org.ziyao.data.util.StreamUtils.toMultiMap(Method::getName, m -> new Function(m, target)));
        }
