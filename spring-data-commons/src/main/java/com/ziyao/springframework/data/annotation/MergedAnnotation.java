...
/**
 * A single merged annotation returned from a {@code MergedAnnotations}
 ...
 * For example, to access an {@code int} attribute the {@code #getInt(String)}
 * method would be used.
 *
 * <p>Note that attribute values are <b>not</b> converted when accessed.
 * For example, it is not possible to call {@code #getString(String)} if the
 ...
 *
 * <p>If necessary, a {@code MergedAnnotation} can be {@codeplain #synthesize()
 * synthesized} back into an actual {@code Annotation}.
 *
...
public interface MergedAnnotation<A extends Annotation> {
     ...
     * {@codeplain #isDirectlyPresent() directly present} and
     * {@codeplain #isMetaPresent() meta-present} annotations within the context
     * of the {@code SearchStrategy} used.
     *
     ...
     * declared and not one that is {@codeplain #isMetaPresent() meta-present}
     * or {@code Inherited @Inherited}.
     *
     ...
     * meta-annotation has a distance of {@code 2}, etc. A {@codeplain #missing()
     ...
     * {@codeplain #missing() missing} annotation will always return an aggregate
     * index of {@code -1}.
     *
     ...
     * {@code MergedAnnotations#from(AnnotatedElement) from} an
     * {@code AnnotatedElement} then this source will be an element of the same
     ...
     * annotation is not {@codeplain #isMetaPresent() meta-present}.
    ...
    /**
     * Get the root annotation, i.e. the {@code #getDistance() distance} {@code 0}
     ...
     * from this annotation to the {@code #getRoot() root}.
     ...
     * @return the value as a {@code MergedAnnotation}
     * @throws NoSuchElementException if there is no matching attribute
     */
     ...
     * @param attributeName the attribute name
     * @param type          the annotation type
     * @return the value as a {@code MergedAnnotation} array
     * @throws NoSuchElementException if there is no matching attribute
     */
     ...
     *
     * @param attributeName the attribute name
     * @return an optional value or {@code Optional#empty()} if there is no
     * matching attribute
     */
     ...
     *
     * @param attributeName the attribute name
     * @return an optional of the default value or {@code Optional#empty()} if
     ...
     * rules applied. Aliases to {@code #getMetaSource() meta-source} attributes will
     * not be applied.
     *
    ...
    /**
     * Create a new mutable {@code com.ziyao.springframework.data.annotation.AnnotationAttributes} instance from this
     * merged annotation.
     * <p>The {@code Adapt adaptations} may be used to change the way that values
     * are added.
     *
    ...
    /**
     * Get an immutable {@code Map} that contains all the annotation attributes.
     * <p>The {@code Adapt adaptations} may be used to change the way that values are added.
     *
     * @param adaptations the adaptations that should be applied to the annotation values
     * @return an immutable map containing the attributes and values
     */
    ...
    /**
     * Create a new {@code Map} instance of the given type that contains all the annotation
     * attributes.
     * <p>The {@code Adapt adaptations} may be used to change the way that values are added.
     *
     ...
     * <p>The result is synthesized using a JDK {@code java.lang.reflect.Proxy Proxy}
     * and as a result may incur a computational cost when first invoked.
     * <p>If this merged annotation was created {@codeplain #of(AnnotatedElement, Class, Map)
     ...
     * <p>If this merged annotation was created {@codeplain #from(Annotation) from}
     ...
     * <ul>
     * <li>The annotation declares attributes annotated with {@code AliasFor @AliasFor}.</li>
     ...
     * <p>The result is synthesized using a JDK {@code java.lang.reflect.Proxy Proxy}
     * and as a result may incur a computational cost when first invoked.
     * <p>Consult the documentation for {@code #synthesize()} for an explanation
    ...
    /**
     * Create a {@code MergedAnnotation} that represents a missing annotation
    ...
    /**
     * Create a new {@code MergedAnnotation} instance from the specified
     * annotation.
     *
     * @param annotation the annotation to include
     * @return a {@code MergedAnnotation} instance containing the annotation
     */
     ...
     * @param annotation the annotation to include
     * @return a {@code MergedAnnotation} instance for the annotation
     */
    ...
    /**
     * Create a new {@code MergedAnnotation} instance of the specified
     ...
     * @return a {@code MergedAnnotation} instance for the annotation and attributes
     ...
     * {@codeplain MergedAnnotation#asMap(Adapt...) Maps} or
     * {@code MergedAnnotation#asAnnotationAttributes(Adapt...) AnnotationAttributes}.
     */
    enum Adapt {
        ...
        /**
         * Factory method to create an {@code Adapt} array from a set of boolean flags.
         *
         * @param classToString    if {@code Adapt#CLASS_TO_STRING} is included
         * @param annotationsToMap if {@code Adapt#ANNOTATION_TO_MAP} is included
         * @return a new {@code Adapt} array
         */
