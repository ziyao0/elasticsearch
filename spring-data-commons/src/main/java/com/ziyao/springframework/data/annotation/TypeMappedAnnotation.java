...
/**
 * {@code MergedAnnotation} that adapts attributes from a root annotation by
 * applying the mapping and mirroring rules of an {@code AnnotationTypeMapping}.
 *
 ...
 * actual {@code Annotation} instance where methods on the annotation instance
 * are {@codeplain com.ziyao.springframework.data.annotation.AnnotationUtils#invokeAnnotationMethod(Method, Object) invoked}
 * to extract values. Similarly, the source could be a simple {@code Map} with
 * values extracted using {@code Map#get(Object)}.
 *
...
final class TypeMappedAnnotation<A extends Annotation> extends AbstractMergedAnnotation<A> {
    ...
    static {
        EMPTY_ARRAYS = new HashMap<Class<?>, Object>() {{
     ...
     * {@codeplain AnnotationTypeMapping#isSynthesizable() synthesizable} in general.
     *
    ...
    private boolean isSynthesizable(Annotation annotation) {
        // Already synthesized?
        if (com.ziyao.springframework.data.annotation.AnnotationUtils.isSynthesizedAnnotation(annotation)) {
            return false;
        }
    ...
    private Object getValueFromMetaAnnotation(int attributeIndex, boolean forMirrorResolution) {
        ...
        if (value == null) {
            Method attribute = this.mapping.getAttributes().get(attributeIndex);
            value = com.ziyao.springframework.data.annotation.AnnotationUtils.invokeAnnotationMethod(attribute, this.mapping.getAnnotation());
        }
        return value;
    }
    ...
    private MergedAnnotation<?> adaptToMergedAnnotation(Object value, Class<? extends Annotation> annotationType) {
        if (value instanceof MergedAnnotation<?>) {
            MergedAnnotation mergedAnnotation = (MergedAnnotation) value;
    ...
    private ValueExtractor getValueExtractor(Object value) {
        if (value instanceof Annotation) {
            return com.ziyao.springframework.data.annotation.AnnotationUtils::invokeAnnotationMethod;
        }
    ...
    private ClassLoader getClassLoader() {
        ...
        if (this.source != null) {
            if (this.source instanceof Class<?>) {
                Class<?> clazz = (Class<?>) this.source;
                return clazz.getClassLoader();
            }
            if (this.source instanceof Member) {
                Member member = (Member) this.source;
    ...
    static <A extends Annotation> TypeMappedAnnotation<A> createIfPossible(
        ...
        return createIfPossible(mapping, source, annotation,
                com.ziyao.springframework.data.annotation.AnnotationUtils::invokeAnnotationMethod, aggregateIndex, logger);
    }
    ...
    private static <A extends Annotation> TypeMappedAnnotation<A> createIfPossible(
        ...
        try {
            ...
            com.ziyao.springframework.data.annotation.AnnotationUtils.rethrowAnnotationConfigurationException(ex);
            if (logger.isEnabled()) {
