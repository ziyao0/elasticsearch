 ...
 * from a source such as a {@code Class} or {@code Method}.
 *
 ...
 *
 * <ul>
 * <li>Explicit and Implicit {@code com.ziyao.springframework.data.annotation.AliasFor @AliasFor} declarations on one or
 * more attributes within the annotation</li>
 * <li>Explicit {@code AliasFor @AliasFor} declarations for a meta-annotation</li>
 ...
 * </table>
 *
 * <p>{@code MergedAnnotations} can be obtained {@codeplain #from(AnnotatedElement)
 * from} any Java {@code AnnotatedElement}. They may also be used for sources that
 * don't use reflection (such as those that directly parse bytecode).
 *
 * <p>Different {@codeplain SearchStrategy search strategies} can be used to locate
 * related source elements that contain the annotations to be aggregated. For
 * example, the following code uses {@code SearchStrategy#TYPE_HIERARCHY} to
 ...
 * {@codeplain #get(String) get} a single annotation, or {@codeplain #stream()
 * stream all annotations} or just those that match {@codeplain #stream(String)
 * a specific type}. You can also quickly tell if an annotation
 * {@codeplain #isPresent(String) is present}.
 *
 ...
 * please use standard Java reflection or Spring's {@code AnnotationUtils}
...
public interface MergedAnnotations extends Iterable<MergedAnnotation<Annotation>> {
    ...
    /**
     * Get the {@codeplain MergedAnnotationSelectors#nearest() nearest} matching
     * annotation or meta-annotation of the specified type, or
     * {@code MergedAnnotation#missing()} if none is present.
     *
     * @param annotationType the annotation type to get
     * @return a {@code MergedAnnotation} instance
     */
     ...
     *                       {@codeplain MergedAnnotationSelectors#nearest() nearest}
     * @return a {@code MergedAnnotation} instance
     * @see MergedAnnotationPredicates
     * @see MergedAnnotationSelectors
     */
     ...
     * type.
     * <p>The resulting stream follows the same ordering rules as {@code #stream()}.
     *
     ...
     * {@codeplain MergedAnnotation#getAggregateIndex() aggregate index} and then
    ...
    /**
     * Create a new {@code MergedAnnotations} instance containing all
     ...
     * {@code #from(AnnotatedElement, SearchStrategy)} with an appropriate
     * {@code SearchStrategy}.
     *
     ...
     * depending on the {@code SearchStrategy}, related inherited elements.
     *
     * @param element        the source element
    ...
    /**
     * Create a new {@code MergedAnnotations} instance from the specified
     * annotations.
     *
     ...
     * must all be {@code MergedAnnotation#isDirectlyPresent() directly present}
     * and must have an {@code MergedAnnotation#getAggregateIndex() aggregate
     * index} of {@code 0}.
    ...
    /**
     * Find merged annotations using the supplied {@code SearchStrategy} and a
     * fluent API for configuring and performing the search.
     * <p>See {@code Search} for details.
     *
     * @param searchStrategy the search strategy to use
     ...
     * {@code MergedAnnotations} model and performing a search.
     *
     * <ul>
     * <li>Configuration starts with an invocation of
     * {@code MergedAnnotations#search(SearchStrategy)}, specifying which
     * {@code SearchStrategy} to use.</li>
     * <li>Optional configuration can be provided via one of the {@code with*()}
     * methods.</li>
     * <li>The actual search is performed by invoking {@code #from(AnnotatedElement)}
    ...
    final class Search {
         ...
         * {@codeplain Class#getEnclosingClass() enclosing classes}.
         * <p>This feature is disabled by default and is only supported when using
         * {@code SearchStrategy#TYPE_HIERARCHY}.
         * <p>Enclosing classes will be recursively searched if the supplied
         * {@code Predicate} evaluates to {@code true}. Typically, the predicate
         ...
         * {@code org.springframework.util.ClassUtils#isInnerClass(Class) ClassUtils::isInnerClass}
         * as the predicate.</li>
         * <li>To limit the enclosing class search to static nested classes, provide
         * {@code org.springframework.util.ClassUtils#isStaticClass(Class) ClassUtils::isStaticClass}
         * as the predicate.</li>
        ...
        /**
         * Configure the {@code RepeatableContainers} to use.
         * <p>Defaults to {@code RepeatableContainers#standardRepeatables()}.
         *
        ...
        /**
         * Configure the {@code AnnotationFilter} to use.
         * <p>Defaults to {@code AnnotationFilter#PLAIN}.
         *
         * @param annotationFilter an annotation filter used to restrict the
         *                         annotations considered
         * @return this {@code Search} instance for chained method invocations
         ...
         * {@code AnnotatedElement} (such as a {@code Class} or {@code Method}),
         * using the configuration in this {@code Search} instance.
         *
         * @param element the source element
         * @return a new {@code MergedAnnotations} instance containing all
         * annotations and meta-annotations from the specified element and,
         * depending on the {@code SearchStrategy}, related inherited elements
    ...
    /**
     * Search strategies supported by {@code MergedAnnotations#search(SearchStrategy)}
     * as well as {@code MergedAnnotations#from(AnnotatedElement, SearchStrategy)}
     * and variants of that method.
     *
     * <p>Each strategy creates a different set of aggregates that will be
     * combined to create the final {@code MergedAnnotations}.
     */
    enum SearchStrategy {

        /**
         * Find only directly declared annotations, without considering
         * {@code Inherited @Inherited} annotations and without searching
         ...
         * {@code Inherited @Inherited} superclass annotations.
         * <p>This strategy is only really useful when used with {@code Class}
         * types since the {@code Inherited @Inherited} annotation is ignored for
         * all other {@codeplain AnnotatedElement annotated elements}.
         ...
         * <p>This strategy is similar to {@code #INHERITED_ANNOTATIONS} except
         * the annotations do not need to be meta-annotated with
         * {@code Inherited @Inherited}.
         * <p>This strategy does not search implemented interfaces.
         */
        SUPERCLASS,
         ...
         * <p>When combined with {@code Search#withEnclosingClasses(Predicate)},
         * {@codeplain Class#getEnclosingClass() enclosing classes} will also be
         * recursively searched if the supplied {@code Predicate} evaluates to
         * {@code true}.
         * <p>Superclass and enclosing class annotations do not need to be
         * meta-annotated with {@code Inherited @Inherited}.
         */
        TYPE_HIERARCHY
