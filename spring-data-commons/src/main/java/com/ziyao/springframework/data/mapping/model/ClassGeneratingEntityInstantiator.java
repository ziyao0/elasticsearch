...
class ClassGeneratingEntityInstantiator implements EntityInstantiator {
	...
	/*
	 * (non-Javadoc)
	 * @see com.ziyao.springframework.data.convert.EntityInstantiator#createInstance(com.ziyao.springframework.data.mapping.PersistentEntity, com.ziyao.springframework.data.mapping.model.ParameterValueProvider)
	 */
	@Override
	...
	boolean shouldUseReflectionEntityInstantiator(PersistentEntity<?, ?> entity) {
		...
		if (entityCreator instanceof PreferredConstructor) {
			...
			PreferredConstructor<?, ?> persistenceConstructor = (PreferredConstructor<?, ?>) entityCreator;
		...
		if (entityCreator instanceof FactoryMethod) {
			...
			FactoryMethod<?, ?> factoryMethod = (FactoryMethod<?, ?>) entityCreator;
	...
	static class ObjectInstantiatorClassGenerator {
		...
		private void visitCreateMethod(ClassWriter cw, PersistentEntity<?, ?> entity,
			...
			if (entityCreator instanceof PreferredConstructor) {
				visitConstructorCreation((PreferredConstructor<?, ?>) entityCreator, mv, entityTypeResourcePath);
			}

			if (entityCreator instanceof FactoryMethod) {
				visitFactoryMethodCreation((FactoryMethod<?, ?>) entityCreator, mv, entityTypeResourcePath);
			}
