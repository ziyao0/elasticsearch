...
import com.ziyao.springframework.data.annotation.AnnotationConfigurationException;
 ...
 * annotations. The {@code #standardRepeatables()} method provides a default
 * strategy that respects Java's {@code Repeatable @Repeatable} support and
 * should be suitable for most situations.
 *
 * <p>The {@code #of} method can be used to register relationships for
 * annotations that do not wish to use {@code Repeatable @Repeatable}.
 *
 * <p>To completely disable repeatable support use {@code #none()}.
 *
 * @author Phillip Webb
 * @author Sam Brannen
 * @since 5.2
...
public abstract class RepeatableContainers {
    ...
    /**
     * Create a {@code RepeatableContainers} instance that searches using Java's
     * {@code Repeatable @Repeatable} annotation.
     *
     * @return a {@code RepeatableContainers} instance
     */
    ...
    /**
     * Create a {@code RepeatableContainers} instance that uses predefined
     * repeatable and container types.
     * <p>WARNING: the arguments supplied to this method are in the reverse order
     * of those supplied to {@code #and(Class, Class)}.
     *
     * @param repeatable the repeatable annotation type
    ...
    /**
     * Create a {@code RepeatableContainers} instance that does not support any
    ...
    /**
     * Standard {@code RepeatableContainers} implementation that searches using
     * Java's {@code Repeatable @Repeatable} annotation.
     */
    private static class StandardRepeatableContainers extends RepeatableContainers {
        ...
        Annotation[] findRepeatedAnnotations(Annotation annotation) {
            ...
            if (method != null) {
                return (Annotation[]) com.ziyao.springframework.data.annotation.AnnotationUtils.invokeAnnotationMethod(method, annotation);
            }
            ...
            try {
                ...
                if (!returnType.isArray() || returnType.getComponentType() != repeatable) {
                    throw new AnnotationConfigurationException(
                            String.format("Container type [%s] must declare a 'value' attribute for an array of type [%s]"
                                    , container.getName(), repeatable.getName()));
                }
            } catch (AnnotationConfigurationException ex) {
                throw ex;
            } catch (Throwable ex) {
                throw new AnnotationConfigurationException(
                        String.format("Invalid declaration of container type [%s] for repeatable annotation [%s]"
                                , container.getName(), repeatable.getName()), ex);
            }
