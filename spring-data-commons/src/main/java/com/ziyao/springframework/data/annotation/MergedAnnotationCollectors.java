...
/**
 * {@code Collector} implementations that provide various reduction operations for
 * {@code MergedAnnotation} instances.
 *
 * @author Phillip Webb
 * @author Sam Brannen
 * @since 5.2
...
public abstract class MergedAnnotationCollectors {
    ...
    /**
     * Create a new {@code Collector} that accumulates merged annotations to a
     * {@code LinkedHashSet} containing {@codeplain MergedAnnotation#synthesize()
     ...
     * @return a {@code Collector} which collects and synthesizes the
     * annotations into a {@code Set}
     */
    ...
    /**
     * Create a new {@code Collector} that accumulates merged annotations to an
     * {@code Annotation} array containing {@codeplain MergedAnnotation#synthesize()
     * synthesized} versions.
     *
     * @param <A> the annotation type
     * @return a {@code Collector} which collects and synthesizes the
    ...
    /**
     * Create a new {@code Collector} that accumulates merged annotations to a
     * {@code MultiValueMap} with items {@codeplain MultiValueMap#add(Object, Object)
     * added} from each merged annotation
     * {@codeplain MergedAnnotation#asMap(Adapt...) as a map}.
     *
     * @param <A>         the annotation type
     * @param adaptations the adaptations that should be applied to the annotation values
     * @return a {@code Collector} which collects and synthesizes the
     * annotations into a {@code LinkedMultiValueMap}
     * @see #toMultiValueMap(Function, com.ziyao.springframework.data.annotation.MergedAnnotation.Adapt...)
     */
     ...
     *
     * @param <A>         the annotation type
     * @param finisher    the finisher function for the new {@code MultiValueMap}
     * @param adaptations the adaptations that should be applied to the annotation values
     * @return a {@code Collector} which collects and synthesizes the
     * annotations into a {@code LinkedMultiValueMap}
     * @see #toMultiValueMap(com.ziyao.springframework.data.annotation.MergedAnnotation.Adapt...)
     */
    public static <A extends Annotation> Collector<MergedAnnotation<A>, ?, MultiValueMap<String, Object>> toMultiValueMap(
    ...
    /**
     * {@code Collector#combiner() Combiner} for collections.
     * <p>This method is only invoked if the {@code java.util.stream.Stream} is
     * processed in {@codeplain java.util.stream.Stream#parallel() parallel}.
     */
    ...
    /**
     * {@code Collector#combiner() Combiner} for multi-value maps.
     * <p>This method is only invoked if the {@code java.util.stream.Stream} is
     * processed in {@codeplain java.util.stream.Stream#parallel() parallel}.
     */
